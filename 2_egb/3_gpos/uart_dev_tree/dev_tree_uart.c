...
static const struct of_device_id serdev_ids_td3[] = {
	{ .compatible = "brightlight,td3_uart" },
	{ }
};
MODULE_DEVICE_TABLE(of, serdev_ids_td3);
...
static size_t td3_uart_recv(struct serdev_device *serdev, const unsigned char *buffer, size_t size) {
	// Puntero a cadena
	static int i = 0;
	printk(KERN_INFO "%s: Recibi %zu bytes por UART\n", AUTHOR, size);
	// Veo si es un caracter valido
	if(*buffer) {
		// Copio el caracter
		shared_buffer[i++] = *buffer;
	}
	// Verifico si me excedi
	if(i == SHARED_BUFF_SIZE || shared_buffer[i-1] == '\0') {
		// Imprimo el mensaje recibido
		printk(KERN_INFO "%s: Se recibieron %d bytes por UART. El mensaje fue '%s'\n", AUTHOR, i-1, shared_buffer);
		// Reinicio las variables
		memset(shared_buffer, 0, i);
		i = 0;
	}
	return size;
}
